...
function InfoList($num=0, $content='')
{
	$contstr = '';
	if(isset($content))
	{
		if(!empty($num))
		{
			$contstr .= ReStrLen($content, $num);
		}
		else
		{
			return GetContPage($content);
		}
	}
	else
	{
...
function GetNav($pid=1)
{
	...
	while($row = $dosql->GetArray())
	{
		...
		if($i==1){$str .= '<li class="liff one"><a href="'.$gourl.'"';}
		else if($classname=='解决方案'){$str .= '<li class="liff "><a id="fangan" href="'.$gourl.'"';}
		else $str .= '<li class="liff"><a href="'.$gourl.'"';
