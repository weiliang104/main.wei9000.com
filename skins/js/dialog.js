...
var artDialog = function (config, ok, cancel) {
	...
	art.WYCMS_API = art.WYCMS_API || [];
     var that_Api = art.WYCMS_API[config.id];
...
artDialog.fn = artDialog.prototype = {
	...
	content: function (msg,url) {
		var prev, next, parent, display,
			...
			if (isIframe) {
				if (typeof wy_ssud == 'string') url += (url.indexOf('?') > -1 ? '&': '?') + 'wy_ssud=' + wy_ssud;
		...
		if (typeof msg === 'string') {
			...
			if (isIframe) art.WYCMS_API[that.config.id] = msg.contentWindow
		};
